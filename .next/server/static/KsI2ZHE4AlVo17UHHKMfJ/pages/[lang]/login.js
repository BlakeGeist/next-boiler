...
const LoginBase = ({
  ...
  const handleEmailPassAuth = e => {
    ...
    firebase_app__WEBPACK_IMPORTED_MODULE_3___default.a.auth().signInWithEmailAndPassword(email, password).then(response => {
      response.user.getIdToken().then(token => {
        //dispatch({ type: 'SET_ITEM', name: 'user', payload: user });
        // eslint-disable-next-line no-undef
        js_cookie__WEBPACK_IMPORTED_MODULE_12___default.a.set("token2", token, {
          expires: 1
        });
  ...
  return __jsx(_layouts_Layout__WEBPACK_IMPORTED_MODULE_6__[/* default */ "a"], {
    ...
    className: "jsx-2762284588" + " " + "container"
  }, __jsx("h1", {
