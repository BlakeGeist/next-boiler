...
const SignUpBase = ({
  ...
  const handleEmailPassAuth = e => {
    ...
    const db = firebase_app__WEBPACK_IMPORTED_MODULE_3___default.a.firestore();
    firebase_app__WEBPACK_IMPORTED_MODULE_3___default.a.auth().createUserWithEmailAndPassword(email, password).then(async res => {
      const userDataCollection = db.collection("userData");
      await userDataCollection.doc(res.user.uid).set({}).then(() => {}).catch(e => {
        res.json({
          error: e
        });
      });
  ...
  const onAuthStateChange = user => {
    if (user && user.uid) {//Router.push(`/${lang}/dashboard`);
    }
  };
  ...
  return __jsx(_layouts_Layout__WEBPACK_IMPORTED_MODULE_6__[/* default */ "a"], {
    ...
    className: "jsx-1950101003" + " " + "container"
  }, __jsx("h1", {
